...
namespace ndifix{
	...
	public:
		...
		template<typename U>
		friend ndifix::Matrix<U> tensor(ndifix::Matrix<U>&, Matrix<U>&);
	};
	...
	template<class T> ndifix::Matrix<T> tensor(ndifix::Matrix<T> &a, Matrix<T> &b){
	...
	private:
		int size;
		std::vector<std::vector<T>> data=Matrix<T>::data;
	public:
		...
		sqMatrix(std::vector<std::vector<T>> &v){size=v.size();for(int i=0;i<size;i++)if(v[i].size()!=size){sqMatrix();return;}data=v;}
